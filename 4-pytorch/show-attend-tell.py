...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        alpha = self.softmax(att).unsqueeze(2)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        # Flatten feature map: (batch_size, 196, encoder_dim)
        encoder_out = encoder_out.view(batch_size, -1, encoder_dim)  
        num_pixels = encoder_out.size(1)

        # Sort input data by decreasing lengths
        caption_lengths, sort_index = caption_lengths.squeeze().sort(dim=0, descending=True)
        encoder_out = encoder_out[sort_index]
        encoded_captions = encoded_captions[sort_index]

        # (batch_size, max_caption_length, embed_dim)
        word_embed = self.embedding(encoded_captions)

        # Initial LSTM state: (batch_size, decoder_dim)
        ...
        # At each time-step, decode the encoder's output 
        # based on the decoder's previous hidden state output by attention-weighing
        ...
        for t in range(max(decode_lengths)):
            ...
            input = torch.cat((word_embed[:batch_size_t, t, :], context), dim=1)
            h, c = self.LSTMCell(input, (h[:batch_size_t], c[:batch_size_t]))   # (batch_size_t, decoder_dim)
